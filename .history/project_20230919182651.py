...
class ProgramUI:
    ...
    def option_solver(self, prompt="Select Option Number: "):
        while True:
            ...
            if not re.search(r"^\d$", menu_input) or not (
                0 <= int(menu_input) <= (len(self.menu) - 1)
                ):
                print("Invalid Input (Usage: Select Option Number)")
                continue
            else:
                self.option = self.menu[int(menu_input) - 1][0]
                match self.option:
                    case "New Inventory":
                    ...
                    case "Exit Program":
                        return sys.exit("Program Closed")
                    case _:
                        return sys.exit("Program Error")
