...
class ProgramUI:
    ...
    def menu_option_solver(self, prompt='Select Option Number: '):
        while True:
            menu_input = int(nput(prompt))
            if not re.search(r"^\d$", menu_input) or not (0 <= menu_input <= len(self.menu) - 1):
                print("Invalid Input (Usage: Select Option Number)")
                continue
            return self.menu[menu_input - 1][0]
