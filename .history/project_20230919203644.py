    ...
    def option_solver(self, prompt="Select Option Number: "):
        while True:
            ...
            if 0 < int(menu_input) <= (len(self.menu)):
                global ui_transfer
                ui_transfer = self.menu[int(menu_input) - 1][0]
                break
...
class NewInventoryMenu(ProgramUI):
    ...
    def new_inventory(self):
        ...
        with open(f"inventories/{self.inventory_filename}.csv", 'x') as file:
            print(f"New inventory file created named {file}.csv!")
