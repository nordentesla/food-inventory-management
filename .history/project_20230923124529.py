...
class Inventory:
    def __init__(self, inv_name: str, storage: list):
    ...
    def inventory_table(self):
        ...
        print(
            tabulate(self.inv_name, showindex=list(map(lambda x:x+1, list(range(len(self.inv_name))))),
                     tablefmt='simple_grid')
            )
    ...
    def item_option(self):
        while True:
            try:
                user_input = input(f'===> {self.inv_name.upper()} - Input Option Number: ')
                if 0 < int(user_input) <= len(self.ui_menu):
                    return self.ui_menu[int(user_input) - 1][0]
                print("Invalid option, select a valid option number")
                continue
